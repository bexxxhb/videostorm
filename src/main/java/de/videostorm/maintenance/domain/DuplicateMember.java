 ...
 * IMDb id, original title, file path and file size. These are the movie's own raw values as the scan
 * saw them — a snapshot, so the persisted run stays meaningful even after the catalogue changes.
 */
public record DuplicateMember(
    ...
    static DuplicateMember from(ScanCandidate candidate) {
        return new DuplicateMember(
                candidate.imdbId(), candidate.originalTitle(), candidate.filePath(), candidate.sizeBytes());
    }
}
