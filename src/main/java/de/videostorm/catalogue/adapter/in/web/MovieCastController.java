...
/**
 * Serves a single movie's cast on demand, so the listing page never carries the performers for every
 * row. Returns a JSON array in billing order; 404 when the movie is unknown, {@code []} when it has no
 * cast.
 */
@RestController
...
public class MovieCastController {
    ...
    private final MovieCastQuery movieCastQuery;
    ...
    public ResponseEntity<List<ActorResponse>> actors(@PathVariable long id) {
        return movieCastQuery.castFor(id)
